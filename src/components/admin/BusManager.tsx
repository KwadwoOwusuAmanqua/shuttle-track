...
export default function BusManager() {
  ...
  function startEdit(bus: Bus) {
    ...
    setForm({
      ...
      speed: bus.speed,
      dwellRemaining: bus.dwellRemaining,
    });
  }

  function cancelEdit() {
    setEditId(null);
    setForm({ routeId: "", name: "", pathIndex: 0, speed: 0.004, dwellRemaining: 0 });
  }
        ...
        {buses.map((bus) => {
          ...
          return (
            <div key={bus.id} className="list-item">
              <span
                className="color-dot"
                style={{ backgroundColor: route?.color ?? Colors.outline_variant }}
              />
